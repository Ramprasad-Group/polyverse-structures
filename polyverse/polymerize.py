...
from polyverse.constraints import DefaultRingSizeConstraint
...
class Polymerize:
    def __init__(
        ...
        if self.reaction_class == "condensation" and not self.aacondensation:
            self.template_2 = self.rxn.GetReactantTemplate(1)
        elif self.reaction_class == "addition" or self.aacondensation:
...
class Polyimide(Polymerize):
    # TODO: Add backbite constraint
    # TODO: Add charge constraint
    pass
...
class ROMP(Polymerize):
    ...
    def filter_by_ring_size(self, row):
        _id, smiles = row["id-smiles"]
        mol = Chem.MolFromSmiles(smiles)
        # If the molecule is None, then we cannot check the ring size. Set mol = None.
    ...
    def generate_polymers(self, molecule_df):
        ...
        for ind, index_slice in enumerate(index_slices):
            ...
            chunk = daskify_df2df(
                lambda df: df.apply(lambda x: self.filter_by_ring_size(x), axis=1),
                chunk,
                self.npartitions,
            )
